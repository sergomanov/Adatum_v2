...
int ledcol=0;
int pirPin = 9;                  // датчик движения
int dalaypir = 0;                //  состояние дачтика движения
...
void setup () {
  ...
  if (!ether.dhcpSetup())  {  Serial.println("DHCP failed"); resetFunc();}
   //  ether.staticSetup(myip, gwip);
  ether.printIp("IP:  ", ether.myip);
  ether.printIp("GW:  ", ether.gwip);
  ether.printIp("DNS: ", ether.dnsip);  
  if (!ether.dnsLookup(website))  {  Serial.println("DNS failed"); resetFunc(); }// для работы по днс имени
...
if (millis() > timer){ timer = millis() + 5000; ether.browseUrl(PSTR("/post.php?ID="), mack, website, my_callback); }   // Проверка состояния
...
if (millis() > timer2) 
{
     ...
     int zn_lum = analogRead(A0);        
     char Result[100]; sprintf(Result, "ID=%s&TEM=%d&P=%d&LUM=%d&PIR=%d", mack,Tempe,pres,zn_lum,dalaypir); 
     Serial.println(Result);
     ether.browseUrl(PSTR("/post.php?"), Result, website, my_callback);
     dalaypir=0; // обнуляем датчик движения 
}
