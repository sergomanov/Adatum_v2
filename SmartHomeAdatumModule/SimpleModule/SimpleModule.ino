  ...
  static byte mymac[] = { 0x74,0x69,0x69,0x2D,0x30,0x99 };      // менять mac здесь
...
//  static byte gwip[] = { 192,168,0,232 };                     //закоментировать для  работы поDHCP
  char* mack="74-69-69-2D-30-99";                              // менять mac здесь
  byte Ethernet::buffer[700];
  static uint32_t timer;
  char website[] PROGMEM = "online.adatum.ru";
 
static void my_callback (byte status, word off, word len) {Serial.println("...");}
...
//  ether.staticSetup(myip, gwip);                              //закоментировать для  работы поDHCP
  if (!ether.dhcpSetup())    Serial.println("DHCP failed");     //закоментировать для работы по статике
